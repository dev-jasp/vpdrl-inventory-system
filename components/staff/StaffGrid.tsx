...
export function StaffGrid({ rows }: { rows: Staff[] }) {
  if (rows.length === 0) {
    return (
      <p className="rounded-[14px] border border-border bg-surface px-5 py-10 text-center text-[13px] font-medium text-text-3">
      ...
      {rows.map((person) => (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="ml-auto flex-none text-[10px] font-bold tracking-[0.04em] uppercase">
              {person.employment}
            </span>
          </div>

          <div className="mt-3.5 text-[12.5px] font-semibold">
            {person.phone}
          </div>
          <a
